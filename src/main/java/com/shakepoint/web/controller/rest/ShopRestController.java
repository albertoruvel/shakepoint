...
public class ShopRestController {
	...
	@RequestMapping(value="/profile", method=RequestMethod.GET,
			produces="application/json")
	public UserProfile getUserProfile(@AuthenticationPrincipal Principal principal){
		return shopFacade.getUserProfile(principal); 
	}
	
	@RequestMapping(value="/save_profile", method=RequestMethod.POST, 
			produces="application/json", consumes="application/json")
	public @ResponseBody UserProfile saveUserProfile(@AuthenticationPrincipal Principal p, @RequestBody UserProfileRequest request){
		return shopFacade.saveProfile(p, request); 
	}
	
	@RequestMapping(value="/combos", method=RequestMethod.GET, 
			produces="application/json")
	public @ResponseBody List<Combo> getCombos(@RequestParam(value="machine_id", required=true)String machineId,
											   @RequestParam(value="page_number", required=false, defaultValue="1")int pageNumber){
